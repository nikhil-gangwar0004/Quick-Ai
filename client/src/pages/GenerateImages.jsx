  ...
  return (
    <div className='h-full overflow-y-scroll p-6 flex items-start flex-wrap gap-4 text-slate-700'>
      <form onSubmit={onSubmitHandler} className='w-full max-w-lg p-4 bg-white rounded-lg border border-gray-200'>
        ...
        <textarea
          onChange={(e) => setInput(e.target.value)} value={input} rows={4}
        ...
        <div className='mt-3 flex gap-3 flex-wrap'>
          {imagestyle.map((item, index) => (
            <span key={index} onClick={() => setSelectedStyle(item)}
        ...
        <button disabled={loading} className='w-full flex justify-center items-center gap-2 bg-linear-to-r from-[#00AD25] to-[#04FF50] text-white px-4 py-2 mt-6 text-sm rounded-lg disabled:opacity-60'>
          <Image className='w-5' />
